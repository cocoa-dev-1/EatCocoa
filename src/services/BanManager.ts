...
export class BanManager {
  ...
  async unban(interaction: CommandInteraction, targetUserId: string) {
    try {
      ...
      this.Error(interaction, `<@${targetUserId}> 님은 차단해제할 수 없습니다.`);
    }
  }
  ...
  async isBanned(interaction: CommandInteraction, targetUserId: string): Promise<User|undefined> {
    const targetUserCash = await interaction.guild.bans.fetch();
    const isBanned = targetUserCash.find(user => user.user.id === targetUserId);
  ...
  canBan(interaction: CommandInteraction, targetUser: User): boolean {
    ...
    if (targetUserCash.roles.highest.comparePositionTo(userCash.roles.highest) >= 0 && !targetUserCash.permissions.has(DiscordPermission.ADMIN)) {
      return true;
    }
    return false;
  }

  async canUnban(interaction: CommandInteraction, targetUserId: string): Promise<boolean> {
    try {
  ...
  async banSuccess(interaction: CommandInteraction, targetUser: User): Promise<void> {
